...
func createAccountRecord(ctx context.Context, client *dbent.Client, account *service.Account) error {
	...
	upstreamGroupName, upstreamGroupID, err := resolveOrCreateUpstreamGroupWithClient(ctx, client, account.UpstreamGroup)
	if err != nil {
		return err
	}
...
func lockAndMergeAccountProbeExtra(
	...
	if identityUnchanged && intervalUnchanged && autoSyncUnchanged && !probeExplicitlyDisabled {
...
func (r *accountRepository) resolveOrCreateUpstreamGroup(ctx context.Context, raw string) (string, *int64, error) {
	...
	return resolveOrCreateUpstreamGroupWithClient(ctx, r.client, raw)
}

func resolveOrCreateUpstreamGroupWithClient(ctx context.Context, client *dbent.Client, raw string) (string, *int64, error) {
	...
	if err := client.AccountUpstreamGroup.Create().
		SetName(name).
	...
	group, err := client.AccountUpstreamGroup.Query().
...
func (r *accountRepository) updateUpstreamBillingProbeSnapshotInTx(
	...
	var expectedLegacyRateSyncEnabled any
	if account.Extra != nil {
		expectedLegacyRateSyncEnabled = account.Extra[service.UpstreamBillingProbeAutoSyncExtraKey]
	}
	expectedLegacyRateSyncEnabledJSON, err := json.Marshal(expectedLegacyRateSyncEnabled)
	if err != nil {
		return err
	}
	client := clientFromContext(ctx, r.client)
	...
	result, err := client.ExecContext(ctx, `
			...
			rate_multiplier = CASE
				WHEN $11::numeric IS NOT NULL
					AND (
						...
						OR extra @> '{"upstream_billing_probe_auto_sync_rate_multiplier": true}'::jsonb
					)
				THEN $11::numeric
		...
		WHERE id = $2
			...
			AND COALESCE(extra -> 'upstream_billing_probe_auto_sync_rate_multiplier', 'null'::jsonb) = $10::jsonb
			AND deleted_at IS NULL
	`, string(payload), account.ID, account.Platform, account.Type, string(credentials), proxyID, string(expectedSnapshotJSON), string(expectedEnabledJSON), string(expectedRateSyncEnabledJSON), string(expectedLegacyRateSyncEnabledJSON), rateMultiplier)
	if err != nil {
		return err
	}
	affected, err := result.RowsAffected()
