	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
				...
				WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "snapshot", "interval", "auto_sync", "ollama_session", "ollama_auto", "ollama_snapshot"}).
					AddRow(tt.identityUnchanged, false, true, tt.databaseEnabled, tt.databaseSnapshot, tt.databaseInterval, tt.databaseAutoSync, nil, nil, nil))
