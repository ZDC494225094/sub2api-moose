...
type AdminPaymentOrderResult struct {
	RechargeCampaign    *service.RechargeCampaignSnapshot `json:"recharge_campaign,omitempty"`
	ID                  int64                             `json:"id"`
...
func sanitizeAdminPaymentOrderForResponseWithPlanNames(order *dbent.PaymentOrder, planNames map[int64]string) *AdminPaymentOrderResult {
	...
	return &AdminPaymentOrderResult{
		RechargeCampaign:    service.PaymentOrderRechargeCampaign(order),
		ID:                  order.ID,
		UserID:              order.UserID,
...
type AdminSubscriptionPlanResult struct {
	...
	DisplayPurchaseCount int       `json:"display_purchase_count"`
...
func adminSubscriptionPlansForResponse(plans []*dbent.SubscriptionPlan, groupInfo map[int64]service.PlanGroupInfo) []AdminSubscriptionPlanResult {
	...
	for _, p := range plans {
		...
		result = append(result, AdminSubscriptionPlanResult{
			...
			DisplayPurchaseCount: p.DisplayPurchaseCount,
