...
func TestUpdateExistingSubscriptionTermUsesLockedCurrentRow(t *testing.T) {
	...
	err := svc.updateExistingSubscriptionTerm(context.Background(), 7, 5, "renewed", false)
	require.NoError(t, err)
	sub, err := repo.GetByID(context.Background(), 7)
...
func TestUpdateExistingSubscriptionTermSerializedRenewalsAccumulateDays(t *testing.T) {
	now := time.Date(2026, 8, 2, 12, 0, 0, 0, time.UTC)
	...
	svc.now = func() time.Time { return now }
	err := svc.updateExistingSubscriptionTerm(context.Background(), 17, 7, "", false)
	require.NoError(t, err)
	err = svc.updateExistingSubscriptionTerm(context.Background(), 17, 7, "", false)
	require.NoError(t, err)
	second, err := repo.GetByID(context.Background(), 17)
	require.NoError(t, err)
...
func TestUpdateExistingSubscriptionTermAssignmentSemanticsKeepsSuspendedRow(t *testing.T) {
	now := time.Date(2026, 8, 2, 12, 0, 0, 0, time.UTC)
	windowStart := now.Add(-24 * time.Hour)
	current := UserSubscription{
	...
	err := svc.updateExistingSubscriptionTerm(context.Background(), 27, 5, "renewed", true)
	require.NoError(t, err)
	sub, err := repo.GetByID(context.Background(), 27)
