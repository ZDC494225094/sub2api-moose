...
func TestSettingService_GetPublicSettings_ExposesMainlandChinaAccessRestriction(t *testing.T) {
	svc := NewSettingService(&settingPublicRepoStub{values: map[string]string{
		SettingKeyMainlandChinaAccessRestrictionEnabled: "true",
	}}, &config.Config{})

	settings, err := svc.GetPublicSettings(context.Background())

	require.NoError(t, err)
	require.True(t, settings.MainlandChinaAccessRestrictionEnabled)
}
