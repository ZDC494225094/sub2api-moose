...
func (s *PaymentService) ensurePaymentSubscriptionAssigned(ctx context.Context, o *dbent.PaymentOrder, groupID int64, days int) error {
	...
	if paymentOrderHasRecordedSubscription(o) {
		assignedSubscription, err = s.subscriptionSvc.userSubRepo.GetByID(txCtx, *o.SubscriptionID)
		if err != nil {
			return fmt.Errorf("load recorded subscription %d: %w", *o.SubscriptionID, err)
		}
		if assignedSubscription.UserID != o.UserID || assignedSubscription.GroupID != groupID {
			return fmt.Errorf("recorded subscription %d does not belong to payment order", assignedSubscription.ID)
		}
	} else {
		active, lookupErr := s.subscriptionSvc.userSubRepo.ListActiveByUserIDAndGroupID(txCtx, o.UserID, groupID)
		...
		for i := range active {
			if hasPaymentSubscriptionOrderNote(active[i].Notes, orderNote) {
				assignedSubscription = &active[i]
	...
	if !alreadyAssigned {
		...
		if _, err := txClient.PaymentAuditLog.Create().
			...
			if dbent.IsConstraintError(err) {
				...
				if checkErr == nil && claimed {
					current, getErr := s.entClient.PaymentOrder.Get(ctx, o.ID)
					if getErr == nil && current.SubscriptionID != nil {
						o.SubscriptionID = current.SubscriptionID
					}
		...
		slog.Info("subscription already assigned for order, skipping", "orderID", o.ID, "groupID", groupID, "subscriptionID", assignedSubscription.ID)
	}
	...
	if err := s.subscriptionSvc.invalidateSubscriptionByID(assignedSubscription.ID); err != nil {
		return fmt.Errorf("invalidate exact subscription cache after fulfillment: %w", err)
	}
	return nil
}
