...
// The AccountUpstreamGroupFunc type is an adapter to allow the use of ordinary function as a Querier.
type AccountUpstreamGroupFunc func(context.Context, *ent.AccountUpstreamGroupQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f AccountUpstreamGroupFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.AccountUpstreamGroupQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.AccountUpstreamGroupQuery", q)
}

// The TraverseAccountUpstreamGroup type is an adapter to allow the use of ordinary function as Traverser.
type TraverseAccountUpstreamGroup func(context.Context, *ent.AccountUpstreamGroupQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseAccountUpstreamGroup) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseAccountUpstreamGroup) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.AccountUpstreamGroupQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.AccountUpstreamGroupQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.AccountUpstreamGroupQuery:
		return &query[*ent.AccountUpstreamGroupQuery, predicate.AccountUpstreamGroup, accountupstreamgroup.OrderOption]{typ: ent.TypeAccountUpstreamGroup, tq: q}, nil
