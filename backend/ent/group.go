...
type Group struct {
	...
	// 自动探测后同步分组倍率的 OpenAI API Key 参考账号 ID
	BillingRateSyncAccountID *int64 `json:"billing_rate_sync_account_id,omitempty"`
	// 参考账号上游声明倍率之上的加价倍率
	BillingRateMarkup float64 `json:"billing_rate_markup,omitempty"`
	// 是否启用高峰时段倍率
	PeakRateEnabled bool `json:"peak_rate_enabled,omitempty"`
	// 高峰开始时间 HH:MM（含），如 22:00；空表示未配置；支持跨午夜
	PeakStart string `json:"peak_start,omitempty"`
	// 高峰结束时间 HH:MM（不含），必须与 peak_start 不同；支持跨午夜，如 22:00-02:00
	PeakEnd string `json:"peak_end,omitempty"`
	// 高峰时段最终倍率，仅在 peak_rate_enabled 且处于 [peak_start, peak_end) 时覆盖文本倍率
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldRateMultiplier, group.FieldBillingRateMarkup, group.FieldPeakRateMultiplier, group.FieldDailyLimitUsd, group.FieldWeeklyLimitUsd, group.FieldMonthlyLimitUsd, group.FieldImageRateMultiplier, group.FieldImagePrice1k, group.FieldImagePrice2k, group.FieldImagePrice4k, group.FieldBatchImageDiscountMultiplier, group.FieldBatchImageHoldMultiplier, group.FieldVideoRateMultiplier, group.FieldVideoPrice480p, group.FieldVideoPrice720p, group.FieldVideoPrice1080p, group.FieldWebSearchPricePerCall, group.FieldSearchPricePer1k, group.FieldAudioRealtimePricePerMin, group.FieldAudioTtsPricePerMillionChars, group.FieldAudioSttPricePerHour, group.FieldProfitMinMargin, group.FieldProfitSafetyBuffer:
			values[i] = new(sql.NullFloat64)
		case group.FieldID, group.FieldBillingRateSyncAccountID, group.FieldDefaultValidityDays, group.FieldFallbackGroupID, group.FieldFallbackGroupIDOnInvalidRequest, group.FieldSortOrder, group.FieldRpmLimit:
	...
	for i := range columns {
		...
		case group.FieldBillingRateSyncAccountID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field billing_rate_sync_account_id", values[i])
			} else if value.Valid {
				_m.BillingRateSyncAccountID = new(int64)
				*_m.BillingRateSyncAccountID = value.Int64
			}
		case group.FieldBillingRateMarkup:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field billing_rate_markup", values[i])
			} else if value.Valid {
				_m.BillingRateMarkup = value.Float64
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.BillingRateSyncAccountID; v != nil {
		builder.WriteString("billing_rate_sync_account_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
	builder.WriteString("billing_rate_markup=")
	builder.WriteString(fmt.Sprintf("%v", _m.BillingRateMarkup))
	builder.WriteString(", ")
