...
func (_u *UserAttributeDefinitionUpdate) defaults() error {
	if _, ok := _u.mutation.UpdatedAt(); !ok {
		if userattributedefinition.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("enttmp: uninitialized userattributedefinition.UpdateDefaultUpdatedAt (forgotten import enttmp/runtime?)")
		}
...
func (_u *UserAttributeDefinitionUpdateOne) sqlSave(ctx context.Context) (_node *UserAttributeDefinition, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`enttmp: missing "UserAttributeDefinition.id" for update`)}
	}
