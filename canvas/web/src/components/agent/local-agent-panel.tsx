...
export function LocalAgentPanel({ embedded, headless, autoConnect }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean }) {
    ...
    useEffect(() => {
        ...
        // Canvas Agent 0.6.0 omitted Vary: Origin for some responses. Keep a
        // connection-specific URL so a prior local Canvas session cannot poison
        // this HTTPS deployment's cached CORS response.
        const source = new EventSource(`${endpoint}/events?token=${encodeURIComponent(token)}&clientId=${encodeURIComponent(clientId)}&connection=${encodeURIComponent(crypto.randomUUID())}`);
    ...
    return await Promise.all(
        sources.map(async (source, index) => {
            const blob = source.startsWith("data:image/")
                ...
                : await fetch(`${endpoint}/agent/local-image?token=${encodeURIComponent(token)}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ path: source }) }).then(async (response) => {
