...
import { createModelChannel, isManagedCanvasChannel, modelOptionsFromChannels, normalizeModelOptionValue, selectableModelsByCapability, useConfigStore, type AiConfig, type ApiCallFormat, type ConfigTabKey, type ModelCapability, type ModelChannel } from "@/stores/use-config-store";
...
export function AppConfigPanel({ showDoneButton = false, initialTab = "channels" }: { showDoneButton?: boolean; initialTab?: ConfigTabKey }) {
    ...
    const [managedGroupsExpanded, setManagedGroupsExpanded] = useState(false);
    ...
    const finishConfig = () => {
        const ready = isAiConfigReady(config, config.model || config.imageModel || config.textModel || config.videoModel || config.audioModel);
    ...
    const addChannel = () => {
        const channel = createModelChannel({ name: t("config.channels.numberedName", { count: config.channels.filter((item) => !isManagedCanvasChannel(item)).length + 1 }) });
        updateChannels([...config.channels, channel]);
        setEditingChannelId(channel.id);
    };

    const deleteChannel = (id: string) => {
        const hasManagedChannel = config.channels.some(isManagedCanvasChannel);
        const thirdPartyChannels = config.channels.filter((channel) => !isManagedCanvasChannel(channel));
        if (!hasManagedChannel && thirdPartyChannels.length <= 1) {
                        ...
                        children: (
                            <div>
                                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                                    <div className="text-xs text-stone-500">平台分组无需 API Key；也可以添加第三方 API 渠道。</div>
                                    <Button type="primary" icon={<Plus className="size-4" />} onClick={addChannel}>
                                        添加第三方渠道
                                    </Button>
                                </div>
                                <div className="space-y-2">
                                    {config.channels.some(isManagedCanvasChannel) ? (
                                        <div className="rounded-lg border border-stone-200 dark:border-stone-800">
                                            <button type="button" className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left" onClick={() => setManagedGroupsExpanded((expanded) => !expanded)}>
                                                    <div className="min-w-0">
                                                        <div className="flex min-w-0 items-center gap-2">
                                                            <div className="min-w-0 flex-1 truncate text-sm font-semibold">{siteName}</div>
                                                            <ChannelBadge type="managed" />
                                                        </div>
                                                    <div className="mt-1 truncate text-xs text-stone-500">平台分组 · 登录后按分组计费</div>
                                                </div>
                                                <ChevronDown className={`size-4 shrink-0 text-stone-500 transition-transform ${managedGroupsExpanded ? "rotate-180" : ""}`} />
                                            </button>
                                            {managedGroupsExpanded ? (
                                                <div className="border-t border-stone-200 px-4 py-3 dark:border-stone-800">
                                                    <div className="space-y-3">
                                                        {config.channels.filter(isManagedCanvasChannel).map((channel) => (
                                                            <div key={channel.id} className="min-w-0">
                                                                <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 text-sm">
                                                                    <span className="font-medium">{channel.managedGroup?.name || channel.name || t("config.channels.unnamed")}</span>
                                                                    <span className="text-xs text-stone-500">倍率 {formatRateMultiplier(channel.managedGroup?.rateMultiplier)}</span>
                                                                </div>
                                                                <div className="mt-1 break-words text-xs leading-5 text-stone-500">可用模型：{channel.models.length ? channel.models.map((model) => model.name).join("、") : "暂无"}</div>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
                                    ...
                                    {config.channels.filter((channel) => !isManagedCanvasChannel(channel)).map((channel) => (
                                        <div key={channel.id} className="flex items-center justify-between gap-3 rounded-lg border border-stone-200 px-4 py-3 dark:border-stone-800">
                                            <div className="min-w-0">
                                                <div className="flex min-w-0 items-center gap-2">
                                                    <div className="min-w-0 flex-1 truncate text-sm font-semibold">{channel.name || t("config.channels.unnamed")}</div>
                                                    <ChannelBadge type={isOpenAIChannel(channel) ? "openai" : "thirdParty"} />
                                                </div>
                                                <div className="mt-1 truncate text-xs text-stone-500">{`${apiFormatLabel(channel.apiFormat, t)} · ${t("config.channels.modelCount", { count: channel.models.length })} · ${channel.baseUrl || t("config.channels.missingUrl")}`}</div>
                                            </div>
...
function ChannelBadge({ type }: { type: "managed" | "openai" | "thirdParty" }) {
    const styles = {
        managed: "border-amber-300 bg-amber-100 text-amber-800 dark:border-amber-500/60 dark:bg-amber-500/20 dark:text-amber-200",
        openai: "border-emerald-300 bg-emerald-100 text-emerald-800 dark:border-emerald-500/60 dark:bg-emerald-500/20 dark:text-emerald-200",
        thirdParty: "border-stone-200 bg-stone-100 text-stone-500 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-400",
    };
    const labels = {
        managed: "主站",
        openai: "OpenAI",
    ...
    return <span className={`shrink-0 rounded border px-1.5 py-0.5 text-[10px] font-semibold leading-none ${styles[type]}`}>{labels[type]}</span>;
}

function isOpenAIChannel(channel: ModelChannel) {
    const name = channel.name.trim().toLowerCase();
    const baseUrl = channel.baseUrl.trim().toLowerCase();
    return name === "openai" || name.startsWith("openai ") || name.includes("openai") || baseUrl.includes("api.openai.com");
}

function withChannels(config: AiConfig, channels: ModelChannel[]): AiConfig {
	const fallbackChannel = channels.find((channel) => !isManagedCanvasChannel(channel)) || channels[0];
    const next: AiConfig = {
        ...config,
        channels,
        models: modelOptionsFromChannels(channels),
        baseUrl: fallbackChannel?.baseUrl || config.baseUrl,
        apiKey: fallbackChannel?.apiKey || config.apiKey,
        apiFormat: fallbackChannel?.apiFormat || config.apiFormat,
    };
    return {
        ...next,
...
function formatRateMultiplier(value: number | undefined) {
