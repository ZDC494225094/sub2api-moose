...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    const resizePreviewRef = useRef<{ width: number; height: number; position: Position } | null>(null);
    ...
    const handleResizeMove = useCallback(
        (event: MouseEvent) => {
            ...
            const dx = (event.clientX - resizeRef.current.startX) / safeScale;
            const dy = (event.clientY - resizeRef.current.startY) / safeScale;
            const minWidth = 220;
            const minHeight = 160;
            const maxWidth = 8192;
            const maxHeight = 8192;
            const startWidth = Number.isFinite(resizeRef.current.startWidth) ? resizeRef.current.startWidth : minWidth;
            const startHeight = Number.isFinite(resizeRef.current.startHeight) ? resizeRef.current.startHeight : minHeight;
            const startLeft = Number.isFinite(resizeRef.current.startLeft) ? resizeRef.current.startLeft : 0;
            const startTop = Number.isFinite(resizeRef.current.startTop) ? resizeRef.current.startTop : 0;
            const startRight = startLeft + startWidth;
            const startBottom = startTop + startHeight;
            const fromLeft = resizeRef.current.corner.includes("left");
            const fromTop = resizeRef.current.corner.includes("top");
            const clampDimension = (value: number, min: number, max: number) => Math.min(max, Math.max(min, Number.isFinite(value) ? value : min));
            const rawWidth = clampDimension(startWidth + (fromLeft ? -dx : dx), minWidth, maxWidth);
            const rawHeight = clampDimension(startHeight + (fromTop ? -dy : dy), minHeight, maxHeight);
            let width = rawWidth;
            let height = rawHeight;
            if (resizeRef.current.keepRatio) {
                const ratio = Number.isFinite(resizeRef.current.ratio) && resizeRef.current.ratio > 0 ? resizeRef.current.ratio : 1;
                if (Math.abs(dx) >= Math.abs(dy)) {
                    const ratioLimitedMaxWidth = Math.min(maxWidth, maxHeight * ratio);
                    width = clampDimension(rawWidth, Math.min(minWidth, ratioLimitedMaxWidth), ratioLimitedMaxWidth);
                    height = width / ratio;
                } else {
                    const ratioLimitedMaxHeight = Math.min(maxHeight, maxWidth / ratio);
                    height = clampDimension(rawHeight, Math.min(minHeight, ratioLimitedMaxHeight), ratioLimitedMaxHeight);
                    width = height * ratio;
                }
            }
            ...
            resizePreviewRef.current = { width, height, position };

            // Moving the top or left edge changes the element below the pointer. Rendering the
            // full canvas for every mouse event can repeatedly trigger hover/toolbar state updates.
            // Preview the resize directly and commit the final value once on mouseup instead.
    ...
    const handleResizeUp = useCallback(() => {
        ...
        if (preview) onResize(data.id, preview.width, preview.height, preview.position);
        onResizeEnd(data.id);
    }, [data.id, handleResizeMove, onResize, onResizeEnd]);
...
function ImageContent({
    ...
    const primaryContent = primaryImage?.previewContent || node.metadata?.previewContent || primaryImage?.content || node.metadata?.content;
...
function ExpandedImageCard({ node, image, index, onSetPrimary, onDuplicate, onDownload, onRetry, onDelete }: { node: CanvasNodeData; image: CanvasNodeImage; index: number; onSetPrimary: () => void; onDuplicate: () => void; onDownload: () => void; onRetry: () => void; onDelete: () => void }) {
    ...
    return (
        ...
        >
            {image.content ? <img src={image.previewContent || image.content} alt={node.title} draggable={false} loading="lazy" decoding="async" className="pointer-events-none h-full w-full select-none object-contain" /> : <ImageSlotStatus image={image} />}
            {image.content ? (
