...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: VideoSettingsPanelProps) {
    ...
    const aspectRatio = normalizeVideoAspectRatio(config.videoAspectRatio);
                ...
                <SettingGroup title={t("settingsPanels.video.ratio")} color={theme.node.muted}>
                    <div className="grid grid-cols-2 gap-2.5">
                        {aspectRatioOptions.map((value) => (
                            <OptionPill key={value} selected={aspectRatio === value} theme={theme} onClick={() => onConfigChange("videoAspectRatio", value)}>
                                {value}
                            </OptionPill>
                        ))}
                    </div>
