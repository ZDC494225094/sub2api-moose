...
// `fetch(data:...)` is treated as a connect-src request by strict CSP policies.
// Decode data URLs locally so generated assets never depend on that exception.
export function dataUrlToBlob(dataUrl: string): Blob {
    ...
    if (!isBase64) return new Blob([new TextEncoder().encode(decodeURIComponent(payload))], { type: mimeType });
