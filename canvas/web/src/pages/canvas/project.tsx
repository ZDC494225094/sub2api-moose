...
import { cancelCanvasRun, canvasRunAudioBlob, canvasRunImageDataURLs, canvasRunVideoResult, waitForExistingCanvasRun } from "@/services/api/canvas";
import { defaultConfig, useConfigStore, useEffectiveConfig } from "@/stores/use-config-store";
import { uploadImage } from "@/services/image-storage";
import { uploadMediaFile } from "@/services/file-storage";
import { completedPendingCanvasRuns, registerPendingCanvasRun, removePendingCanvasRuns, restorePendingCanvasRuns } from "@/services/canvas-run-registry";
...
function InfiniteCanvasPage() {
    ...
    const startGenerationRequest = useCallback((targetNodeId: string, originNodeId: string, runningId = originNodeId, controller = new AbortController()) => {
        ...
        generationRequestsRef.current.set(targetNodeId, { targetNodeId, originNodeId, runningNodeId: runningId, controller, runIds: previous?.controller === controller ? previous.runIds : new Set() });
        return controller;
    }, []);
    ...
    const finishGenerationRequest = useCallback((targetNodeId: string, controller: AbortController) => {
        ...
        // The persistence effect removes run IDs only after the terminal node
        // state has been committed to IndexedDB. Keeping the registry until
        // then makes refresh recovery atomic from the user's perspective.
    }, []);

    const rememberGenerationRun = useCallback(async (nodeId: string, runId: string, imageId?: string) => {
        const request = generationRequestsRef.current.get(nodeId);
        if (!request || request.controller.signal.aborted) {
            await cancelCanvasRun(runId).catch(() => undefined);
            return;
        }
        request.runIds.add(runId);
        const persistRunId = (prev: CanvasNodeData[]) =>
            prev.map((node) =>
                node.id !== nodeId
                    ? node
                    : {
                          ...node,
                          metadata: imageId
                              ? { ...node.metadata, images: node.metadata?.images?.map((image) => (image.id === imageId ? { ...image, generationRunId: runId } : image)) }
                              : { ...node.metadata, generationRunId: runId },
                      },
            );
        let nextNodes: CanvasNodeData[] = [];
        let nextConnections = connectionsRef.current;
        flushSync(() => {
            setNodes((prev) => {
                nextNodes = persistRunId(prev);
                nodesRef.current = nextNodes;
                return nextNodes;
            });
            setConnections((prev) => {
                nextConnections = prev;
                connectionsRef.current = prev;
                return prev;
            });
        });
        const project = useCanvasStore.getState().openProject(projectId);
        const pendingNode = nextNodes.find((node) => node.id === nodeId);
        if (pendingNode) {
            registerPendingCanvasRun({
                projectId,
                nodeId,
                imageId,
                runId,
                node: pendingNode,
                connections: nextConnections.filter((connection) => connection.fromNodeId === nodeId || connection.toNodeId === nodeId),
            });
        }
        if (project) useCanvasStore.getState().updateProject(projectId, { nodes: nextNodes, connections: nextConnections });
        await flushCanvasStorePersistence();
    }, [projectId]);

    const setResumedNodes = useCallback((nodeId: string, update: (nodes: CanvasNodeData[]) => CanvasNodeData[]) => {
        setNodes((prev) => {
            const next = update(prev);
            const target = next.find((node) => node.id === nodeId);
            if (!target || target.metadata?.status === NODE_STATUS_LOADING) return next;
            ...
            if (!sourceIds.size) return next;
            return next.map((node) => {
                if (!sourceIds.has(node.id) || node.type !== CanvasNodeType.Config || node.metadata?.status !== NODE_STATUS_LOADING) return node;
                const outputIds = new Set(connectionsRef.current.filter((connection) => connection.fromNodeId === node.id).map((connection) => connection.toNodeId));
                const outputs = next.filter((output) => outputIds.has(output.id));
                if (outputs.some((output) => output.metadata?.status === NODE_STATUS_LOADING)) return node;
                const hasSuccess = outputs.some((output) => output.metadata?.status === NODE_STATUS_SUCCESS);
                const errorDetails = outputs.find((output) => output.metadata?.status === NODE_STATUS_ERROR)?.metadata?.errorDetails;
                return { ...node, metadata: { ...node.metadata, status: hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR, errorDetails: hasSuccess ? undefined : errorDetails } };
            });
        });
    }, []);

    const resumeGenerationRun = useCallback(
        async (nodeId: string, runId: string, imageId?: string) => {
            const requestId = imageId ? `${nodeId}:${imageId}` : nodeId;
            const controller = startGenerationRequest(requestId, nodeId, nodeId);
            generationRequestsRef.current.get(requestId)?.runIds.add(runId);
            setRunningNodeId((current) => current || nodeId);
            try {
                const run = await waitForExistingCanvasRun(runId, controller.signal);
                if (imageId) {
                    const [dataUrl] = await canvasRunImageDataURLs(run, controller.signal);
                    if (!dataUrl) throw new Error(t("canvas.projectPage.generationFailed"));
                    const uploaded = await uploadImage(dataUrl);
                    setResumedNodes(nodeId, (prev) =>
                        prev.map((node) => {
                            ...
                            const item: CanvasNodeImage = {
                                ...
                                previewContent: uploaded.thumbnailUrl,
                            ...
                            const images = node.metadata?.images?.map((image) => (image.id === imageId ? item : image)) || [item];
                            const isPrimary = !node.metadata?.primaryImageId;
                            const imageSize = isPrimary ? fitNodeSize(uploaded.width, uploaded.height, node.width, node.height) : { width: node.width, height: node.height };
                            const stillLoading = images.some((image) => image.status === NODE_STATUS_LOADING);
                            const hasSuccess = images.some((image) => image.status === NODE_STATUS_SUCCESS);
                            return {
                                ...node,
                                ...(isPrimary
                                    ? {
                                ...
                                metadata: {
                                    ...node.metadata,
                                    ...(isPrimary ? imageMetadata(uploaded) : {}),
                                    status: stillLoading ? NODE_STATUS_LOADING : hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR,
                                    errorDetails: undefined,
                                    images,
                                    primaryImageId: isPrimary ? imageId : node.metadata?.primaryImageId,
                                },
                            };
                        }),
                    );
                ...
                if (run.images?.length) {
                    ...
                    setResumedNodes(nodeId, (prev) =>
                        prev.map((node) => {
                            if (node.id !== nodeId) return node;
                            const imageSize = fitNodeSize(uploaded.width, uploaded.height, node.width, node.height);
                            return {
                                ...node,
                                width: imageSize.width,
                                height: imageSize.height,
                                position: { x: node.position.x + node.width / 2 - imageSize.width / 2, y: node.position.y + node.height / 2 - imageSize.height / 2 },
                                metadata: { ...node.metadata, ...imageMetadata(uploaded), generationRunId: undefined, errorDetails: undefined },
                            };
                        }),
                    );
                    return;
                ...
                if (run.videos?.length) {
                    const video = await storeGeneratedVideo(await canvasRunVideoResult(run, controller.signal));
                    const videoSize = fitNodeSize(video.width || VIDEO_NODE_MAX_WIDTH, video.height || VIDEO_NODE_MAX_HEIGHT, VIDEO_NODE_MAX_WIDTH, VIDEO_NODE_MAX_HEIGHT);
                    setResumedNodes(nodeId, (prev) =>
                        prev.map((node) =>
                            node.id === nodeId
                                ? {
                                      ...
                                      metadata: { ...node.metadata, ...videoMetadata(video), generationRunId: undefined, errorDetails: undefined },
                                  }
                                : node,
                        ),
                    );
                ...
                if (run.audios?.length) {
                    const audio = await storeGeneratedAudio(await canvasRunAudioBlob(run, controller.signal));
                    setResumedNodes(nodeId, (prev) => prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, ...audioMetadata(audio), generationRunId: undefined, errorDetails: undefined } } : node)));
                    return;
                }
                ...
                setResumedNodes(nodeId, (prev) =>
                    prev.map((node) =>
                        node.id === nodeId
                            ? { ...node, type: node.type === CanvasNodeType.Config ? CanvasNodeType.Text : node.type, metadata: { ...node.metadata, content: run.content || "", status: NODE_STATUS_SUCCESS, generationRunId: undefined, errorDetails: undefined } }
                            : node,
                    ),
                );
            } catch (error) {
                ...
                setResumedNodes(nodeId, (prev) =>
                    prev.map((node) => {
                        if (node.id !== nodeId) return node;
                        if (imageId) {
                            const images = node.metadata?.images?.map((image) => (image.id === imageId ? { ...image, status: NODE_STATUS_ERROR, generationRunId: undefined, errorDetails } : image));
                            const stillLoading = images?.some((image) => image.status === NODE_STATUS_LOADING);
                            const hasSuccess = images?.some((image) => image.status === NODE_STATUS_SUCCESS);
                            return { ...node, metadata: { ...node.metadata, status: stillLoading ? NODE_STATUS_LOADING : hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR, errorDetails: hasSuccess ? undefined : errorDetails, images } };
                        }
                        return { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, generationRunId: undefined, errorDetails } };
                    }),
                );
            } finally {
                finishGenerationRequest(requestId, controller);
                const nextRunningId = Array.from(generationRequestsRef.current.values())[0]?.runningNodeId || null;
                setRunningNodeId((current) => (current === nodeId ? nextRunningId : current));
            }
        },
        [finishGenerationRequest, setResumedNodes, startGenerationRequest, t],
    );
    ...
    const stopGenerationByRunningId = useCallback((runningId: string) => {
        ...
        generationRequestsRef.current.forEach((request) => {
            ...
            request.runIds.forEach((runId) => void cancelCanvasRun(runId).catch(() => undefined));
            removePendingCanvasRuns(request.runIds);
        ...
        setNodes((prev) =>
            prev.map((node) =>
                affectedNodeIds.has(node.id) && node.metadata?.status === NODE_STATUS_LOADING
                    ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_IDLE, generationRunId: undefined, errorDetails: undefined, images: node.metadata.images?.map((image) => (image.status === NODE_STATUS_LOADING ? { ...image, status: NODE_STATUS_ERROR, generationRunId: undefined, errorDetails: t("common.requestCanceled") } : image)) } }
                    : node,
            ),
        );
    }, [t]);
    ...
    useEffect(() => {
        ...
        const restore = async () => {
            const pending = restorePendingCanvasRuns(projectId, project.nodes, project.connections);
            const resumableTargetIds = new Set(
                pending.nodes
                    .filter((node) => Boolean(node.metadata?.generationRunId || node.metadata?.images?.some((image) => image.status === NODE_STATUS_LOADING && image.generationRunId)))
                    .map((node) => node.id),
            );
            const resumableSourceIds = new Set(pending.connections.filter((connection) => resumableTargetIds.has(connection.toNodeId)).map((connection) => connection.fromNodeId));
            const restoredNodes = await hydrateCanvasImages(resetInterruptedGeneration(pending.nodes, resumableSourceIds));
            const restoredSessions = await hydrateAssistantImages(project.chatSessions || []);
            nodesRef.current = restoredNodes;
            connectionsRef.current = pending.connections;
            setNodes(restoredNodes);
            setConnections(pending.connections);
            ...
            restoredNodes.forEach((node) => {
                node.metadata?.images?.forEach((image) => {
                    if (image.status === NODE_STATUS_LOADING && image.generationRunId) void resumeGenerationRun(node.id, image.generationRunId, image.id);
                });
                if (node.metadata?.status === NODE_STATUS_LOADING && node.metadata.generationRunId) void resumeGenerationRun(node.id, node.metadata.generationRunId);
            });
        };
        void restore();
    }, [hydrated, navigate, openProject, projectId, resumeGenerationRun]);
    ...
    useEffect(() => {
        ...
        const completedRuns = completedPendingCanvasRuns(projectId, nodes);
        if (!completedRuns.length) return;
        const completedRunIds = new Set(completedRuns.map((record) => record.runId));
        void flushCanvasStorePersistence()
            .then(() => {
                removePendingCanvasRuns(completedRunIds);
                setNodes((prev) =>
                    prev.map((node) => {
                        const nodeRuns = completedRuns.filter((record) => record.nodeId === node.id);
                        if (!nodeRuns.length || !node.metadata) return node;
                        const directRunIDs = new Set(nodeRuns.filter((record) => !record.imageId).map((record) => record.runId));
                        const imageRunIDs = new Set(nodeRuns.filter((record) => record.imageId).map((record) => record.runId));
                        return {
                            ...node,
                            metadata: {
                                ...node.metadata,
                                generationRunId: node.metadata.generationRunId && directRunIDs.has(node.metadata.generationRunId) ? undefined : node.metadata.generationRunId,
                                images: node.metadata.images?.map((image) => (image.generationRunId && imageRunIDs.has(image.generationRunId) ? { ...image, generationRunId: undefined } : image)),
                            },
                        };
                    }),
                );
    ...
    const handleNodeResize = useCallback((nodeId: string, width: number, height: number, position?: Position) => {
        ...
        const nextWidth = Math.round(Math.min(8192, Math.max(220, width)));
        const nextHeight = Math.round(Math.min(8192, Math.max(160, height)));
        setNodes((prev) => {
            ...
            return prev.map((item, index) => index === nodeIndex ? { ...item, width: nextWidth, height: nextHeight, position: nextPosition } : item);
        });
    }, []);
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            try {
                const image = await requestEdit(generationConfig, prompt, [source], { id: `${node.id}-mask`, name: "mask.png", type: "image/png", dataUrl: payload.maskDataUrl }, { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(childId, runId) }).then((items) => items[0]);
        ...
        [effectiveConfig, finishGenerationRequest, isAiConfigReady, message, openConfigDialog, rememberGenerationRun, startGenerationRequest, t],
    );
    ...
    const generateAngleNode = useCallback(
        async (node: CanvasNodeData, params: CanvasImageAngleParams) => {
            ...
            try {
                const image = await requestEdit(
                    ...
                    [{ id: node.id, name: `${node.title || node.id}.png`, type: node.metadata.mimeType || "image/png", dataUrl: node.metadata.content, storageKey: node.metadata.storageKey }],
                    undefined,
                    { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(childId, runId) },
                ).then((items) => items[0]);
                const uploaded = await uploadImage(image.dataUrl);
        ...
        [effectiveConfig, finishGenerationRequest, openConfigDialog, rememberGenerationRun, startGenerationRequest, t],
    );
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            if (sourceNode && builtinPanel?.writeBackToSelf && builtinPanel.mode === "image") {
                ...
                try {
                    ...
                    const image = refs.length
                        ? await requestEdit({ ...generationConfig, count: "1" }, fullPrompt, refs, undefined, { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(nodeId, runId) }).then((items) => items[0])
                        : await requestGeneration({ ...generationConfig, count: "1" }, fullPrompt, { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(nodeId, runId) }).then((items) => items[0]);
                    const uploaded = await uploadImage(image.dataUrl);
                    setNodes((prev) =>
            ...
            try {
                        ...
                        imageIds.map(async (imageId) => {
                            try {
                                const image = referenceImages.length
                                    ? await requestEdit({ ...generationConfig, count: "1" }, effectivePrompt, referenceImages, undefined, { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(rootId, runId, imageId) }).then((items) => items[0])
                                    : await requestGeneration({ ...generationConfig, count: "1" }, effectivePrompt, { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(rootId, runId, imageId) }).then((items) => items[0]);
                                const uploaded = await uploadImage(image.dataUrl);
                                const imageSize = fitNodeSize(uploaded.width, uploaded.height, imageConfig.width, imageConfig.height);
                                const item: CanvasNodeImage = { id: imageId, status: NODE_STATUS_SUCCESS, content: uploaded.url, previewContent: uploaded.thumbnailUrl, storageKey: uploaded.storageKey, thumbnailStorageKey: uploaded.thumbnailStorageKey, naturalWidth: uploaded.width, naturalHeight: uploaded.height, bytes: uploaded.bytes, mimeType: uploaded.mimeType };
                                setNodes((prev) =>
                                    prev.map((node) => {
                ...
                if (mode === "video") {
                    const spec = nodeSizeFromRatio(generationConfig.videoAspectRatio, NODE_DEFAULT_SIZE[CanvasNodeType.Video].width, NODE_DEFAULT_SIZE[CanvasNodeType.Video].height) || NODE_DEFAULT_SIZE[CanvasNodeType.Video];
                    ...
                    try {
                        const video = await storeGeneratedVideo(
                            await requestVideoGeneration(generationConfig, effectivePrompt, generationContext.referenceImages, generationContext.referenceVideos, generationContext.referenceAudios, { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(videoId, runId) }),
                        );
                ...
                if (mode === "audio") {
                    ...
                    try {
                        const audio = await storeGeneratedAudio(await requestAudioGeneration(generationConfig, effectivePrompt, { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(audioId, runId) }), generationConfig.audioFormat);
                ...
                const answers = await Promise.all(
                    textTargetIds.map((targetNodeId) => {
                        ...
                        return requestImageQuestion(
                            ...
                            { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(targetNodeId, runId) },
                        )
            ...
            try {
                if (node.type === CanvasNodeType.Text) {
                    ...
                    const answer = await requestImageQuestion(
                        ...
                        { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(node.id, runId) },
                    );
                    setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, type: CanvasNodeType.Text, metadata: { ...item.metadata, content: answer || streamed, prompt, status: NODE_STATUS_SUCCESS } } : item)));
                    return;
                }
                if (node.type === CanvasNodeType.Video) {
                    const video = await storeGeneratedVideo(await requestVideoGeneration(generationConfig, prompt, retryImages, context?.referenceVideos || [], context?.referenceAudios || [], { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(node.id, runId) }));
                ...
                if (node.type === CanvasNodeType.Audio) {
                    const audio = await storeGeneratedAudio(await requestAudioGeneration(generationConfig, prompt, { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(node.id, runId) }), generationConfig.audioFormat);
                    setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, ...audioMetadata(audio), prompt, ...buildAudioGenerationMetadata(generationConfig) } } : item)));
                    return;
                }

                const image = useReferenceImages
                    ? await requestEdit(generationConfig, prompt, retryImages, undefined, { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(node.id, runId, imageId) }).then((items) => items[0])
                    : await requestGeneration(generationConfig, prompt, { signal: controller.signal, onRunStarted: (runId) => rememberGenerationRun(node.id, runId, imageId) }).then((items) => items[0]);
                ...
                const retryImage: CanvasNodeImage = {
                    ...
                    previewContent: uploadedImage.thumbnailUrl,
                    storageKey: uploadedImage.storageKey,
                    thumbnailStorageKey: uploadedImage.thumbnailStorageKey,
