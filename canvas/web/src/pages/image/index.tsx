...
import { deleteStoredImages, resolveImageThumbnailUrl, resolveImageUrl, uploadImage } from "@/services/image-storage";
...
export default function ImagePage() {
    ...
    const addResultToReferences = async (image: GeneratedImage, index: number) => {
        ...
        setReferences((value) => [
            ...value,
            {
                ...
                type: image.mimeType || stored?.mimeType || "image/png",
                dataUrl: stored?.url || image.dataUrl,
                storageKey: stored?.storageKey || image.storageKey,
            },
        ]);
    ...
    const saveResultToAssets = async (image: GeneratedImage, index: number) => {
        ...
        addAsset({
            ...
            coverUrl: stored?.thumbnailUrl || image.thumbnailDataUrl || stored?.url || image.dataUrl,
            tags: [],
            ...
            data: {
                ...
                mimeType: stored?.mimeType || image.mimeType || "image/png",
            },
    ...
    const runGenerationSlot = async (index: number, snapshot: { text: string; config: AiConfig; references: ReferenceImage[] }): Promise<GeneratedImage> => {
        ...
        try {
            ...
            const nextImage: GeneratedImage = {
                ...
                thumbnailDataUrl: stored.thumbnailUrl,
...
function ResultImageCard({
    ...
    return (
        <div className="overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
            <Image src={image.thumbnailDataUrl || image.dataUrl} preview={{ src: image.dataUrl }} loading="lazy" decoding="async" alt={t("imageWorkbench.resultAlt", { count: index + 1 })} className="aspect-square object-cover" />
...
async function normalizeLog(log: Partial<GenerationLog>): Promise<GenerationLog> {
    ...
    const images = await Promise.all(
        (log.images || []).map(async (item) => {
            ...
            const thumbnailStorageKey = item.thumbnailStorageKey || (item.storageKey ? `${item.storageKey}:thumb` : undefined);
            const thumbnailDataUrl = await resolveImageThumbnailUrl(thumbnailStorageKey, item.thumbnailDataUrl || "", item.storageKey);
            return { ...item, dataUrl, thumbnailDataUrl: thumbnailDataUrl || dataUrl, thumbnailStorageKey };
        }),
    );
    ...
    return {
        ...
        images,
        thumbnails: images.map((image) => image.thumbnailDataUrl || image.dataUrl).filter(Boolean),
    };
}
...
function serializeLog(log: GenerationLog): GenerationLog {
    return {
        ...
        images: log.images.map((image) => ({ ...image, dataUrl: image.storageKey ? "" : image.dataUrl, thumbnailDataUrl: image.thumbnailStorageKey ? "" : image.thumbnailDataUrl })),
        thumbnails: [],
    };
}
