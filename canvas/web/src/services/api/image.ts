...
import { buildApiUrl, isManagedCanvasModel, resolveModelRequestConfig, resolveModelScript, type AiConfig, type ModelChannel } from "@/stores/use-config-store";
...
import { canvasRunImageDataURLs, waitForCanvasRun } from "./canvas";
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    if (isManagedCanvasModel(config, selectedModel)) return requestManagedCanvasImages(config, selectedModel, requestConfig.model, prompt, [], n, options);
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    if (isManagedCanvasModel(config, selectedModel)) {
        const images = [...references, ...(mask ? [mask] : [])];
        return requestManagedCanvasImages(config, selectedModel, requestConfig.model, requestPrompt, images, n, options);
    }
    const script = resolveModelScript(config, selectedModel);
    if (script) {
        const quality = normalizeQuality(config.quality);
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    const requestConfig = resolveModelRequestConfig(config, selectedModel);
    if (isManagedCanvasModel(config, selectedModel)) {
        const run = await waitForCanvasRun(
            config,
            {
                mode: "chat",
                model: requestConfig.model,
        ...
        const answer = run.content || apiText("noContent");
        onDelta(answer);
        return answer;
    }
    const script = resolveModelScript(config, selectedModel);
...
async function requestManagedCanvasImages(config: AiConfig, selectedModel: string, model: string, prompt: string, references: ReferenceImage[], count: number, options?: RequestOptions) {
    ...
    const dataUrls = await canvasRunImageDataURLs(run, options?.signal);
    if (!dataUrls.length) throw new Error(apiText("noImageReturned"));
