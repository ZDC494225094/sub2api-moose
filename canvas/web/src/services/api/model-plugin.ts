...
function createPoll(signal?: AbortSignal) {
    return async function poll<T, R>(request: () => Promise<T>, extract: (value: T) => R | null | undefined | false, options?: PluginPollOptions): Promise<R> {
        ...
        const deadline = options?.timeoutMs && options.timeoutMs > 0 ? performance.now() + options.timeoutMs : null;
        for (;;) {
            if (signal?.aborted) throw new DOMException("Aborted", "AbortError");
            const result = extract(await request());
            if (result !== null && result !== undefined && result !== false) return result;
            if (deadline !== null && performance.now() >= deadline) throw new Error(i18n.t("modelPlugin.pollTimeout"));
 ...
 *   http / request / poll / sleep / signal / onDelta / dataUrlToBlob — request helpers
