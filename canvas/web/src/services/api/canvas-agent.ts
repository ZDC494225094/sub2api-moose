...
type AgentConfigResponse = { ok?: boolean; protocolVersion?: number; version?: string; url?: string; token?: string; hasToken?: boolean };
...
export async function discoverAgentConfig(endpoint: string) {
    try {
        // Keep this response uncacheable and URL-unique because it varies by Origin.
        const res = await fetch(`${endpoint}/config?connection=${encodeURIComponent(crypto.randomUUID())}`, { cache: "no-store" });
