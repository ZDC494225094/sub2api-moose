...
export function startHttpServer() {
    ...
    app.get("/config", (req, res) => {
        // This response varies by browser Origin. Never let a response for a
        // localhost page poison a later HTTPS canvas connection.
        res.setHeader("Cache-Control", "no-store");
        res.json({ ok: true, protocolVersion: AGENT_PROTOCOL_VERSION, version: VERSION, url: config.url, hasToken: true });
    });
    app.use((req, res, next) => {
