...
+ [新增] 接入 Sub2API 登录分组调用：画布可使用平台受管模型完成文本、图像、视频和音频生成并按现有账单计费，同时保留第三方 API 渠道配置。
+ [修复] 平台受管生成任务会持久化后台任务编号，刷新画布后继续恢复文本、图片、视频和音频结果，不再把仍在上游执行的任务误判为中断。
+ [修复] 画布移除违反站点 CSP 的内联主题脚本，并避免 Canvas Agent 跨来源缓存 CORS 响应导致线上页面无法连接本地服务。
+ [修复] Canvas Agent 发布版按实际网页来源返回跨域响应，画布标题和受管渠道名称读取站点名称，并修正模型厂商图标的部署路径。
