...
export function videoCount(row: Pick<MediaBillingRow, 'video_count' | 'video_duration_seconds' | 'billing_mode'> | null | undefined): number {
  if (!row) return 0
  if ((row.video_count ?? 0) > 0) return row.video_count ?? 0
  return row.billing_mode === BILLING_MODE_VIDEO || (row.video_duration_seconds ?? 0) > 0 ? 1 : 0
}

export function isVideoUsage(row: Pick<MediaBillingRow, 'video_count' | 'video_duration_seconds' | 'billing_mode'> | null | undefined): boolean {
  return videoCount(row) > 0
}

export function isImageUsage(row: Pick<MediaBillingRow, 'image_count' | 'video_count' | 'video_duration_seconds' | 'billing_mode'> | null | undefined): boolean {
...
export function videoUnitPrice(row: Pick<MediaBillingRow, 'video_count' | 'video_duration_seconds' | 'total_cost' | 'billing_mode'> | null): number {
  const count = videoCount(row)
  if (!row || count <= 0) return 0
  const durationSeconds = row.video_duration_seconds && row.video_duration_seconds > 0 ? row.video_duration_seconds : 1
  const price = (row.total_cost ?? 0) / (count * durationSeconds)
  return Number.isFinite(price) ? price : 0
}
