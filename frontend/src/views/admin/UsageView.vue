...
import { isVideoUsage, videoUnitPrice } from '@/utils/billingMode'
...
const breakdownFilters = computed(() => {
  ...
  if (filters.value.canvas_managed != null) f.canvas_managed = filters.value.canvas_managed
  return f
})
...
const filters = ref<AdminUsageQueryParams>({ user_id: undefined, model: undefined, group_id: undefined, request_type: undefined, native_compaction_v2: null, billing_type: null, billing_mode: null, canvas_managed: null, start_date: startDate.value, end_date: endDate.value })
...
const loadModelStats = async (source: ModelDistributionSource, force = false) => {
  ...
  try {
    ...
    const baseParams = {
	  ...
	  canvas_managed: filters.value.canvas_managed,
...
const resetFilters = () => {
  ...
  filters.value = { start_date: startDate.value, end_date: endDate.value, request_type: undefined, native_compaction_v2: null, billing_type: null, billing_mode: null, canvas_managed: null }
  granularity.value = getGranularityForRange(startDate.value, endDate.value)
  applyFilters()
}
...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('usage.videoCount'), t('usage.videoDuration'), t('usage.videoResolution'), t('usage.videoUnitPrice'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => {
        const videoUsage = isVideoUsage(log)
        return [
          ...
          videoUsage ? '' : log.input_tokens, videoUsage ? '' : log.output_tokens,
          videoUsage ? '' : log.cache_read_tokens, videoUsage ? '' : log.cache_creation_tokens,
          videoUsage ? log.video_count : '', videoUsage ? log.video_duration_seconds ?? '' : '',
          videoUsage ? log.video_resolution || '' : '', videoUsage ? videoUnitPrice(log).toFixed(6) : '',
...
const allColumns = computed(() => [
  ...
  { key: 'stream', label: `${t('usage.type')} / ${t('admin.usage.requestResult')}`, sortable: false },
