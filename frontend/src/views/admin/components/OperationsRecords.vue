<template>
  <section v-if="selection" ref="section" tabindex="-1" class="records-panel rounded-2xl border border-primary-200 bg-white p-5 shadow-sm dark:border-primary-900 dark:bg-dark-800" aria-label="页内分析明细">
    <div class="mb-5 flex flex-wrap items-start justify-between gap-3">
      <div><p class="mb-1 text-xs font-medium text-primary-600">运营分析 / {{ selection.mode === 'orders' ? '成功充值' : '消耗追踪' }}</p><h2 class="text-base font-semibold">{{ selection.title }}</h2><p class="mt-2 text-xs text-gray-500">{{ selection.start_date }} — {{ selection.end_date }} · {{ timezone }} · {{ selection.mode === 'orders' ? '订单创建时间 · 已支付 / 充值中 / 已完成 · USD 额度，非现金实收' : '请求时间 · 当前筛选下的逐笔计费记录' }}</p></div>
      <button class="btn btn-secondary" aria-label="关闭页内明细" @click="close"><Icon name="x" size="sm" /></button>
    </div>
    ...
    <template v-else>
      <div class="overflow-x-auto">
        <table v-if="selection.mode === 'orders'" class="w-full whitespace-nowrap text-sm">
          <thead class="bg-gray-50 text-xs text-gray-500 dark:bg-dark-900"><tr><th class="p-3 text-left">订单号 / 用户</th><th class="p-3 text-left">充值类型</th><th class="p-3 text-left">支付方式</th><th class="p-3 text-right">充值额度 USD</th><th class="p-3 text-left">当前状态</th><th class="p-3 text-left">创建时间</th><th class="p-3 text-left">支付时间</th></tr></thead>
          <tbody><tr v-for="order in orders" :key="order.id" class="border-b border-gray-100 dark:border-dark-700"><td class="p-3"><span class="font-mono text-xs">{{ order.out_trade_no }}</span><span class="mt-1 block text-xs text-gray-500">用户 #{{ order.user_id }}</span></td><td class="p-3">{{ order.order_type === 'subscription' ? '订阅充值' : '余额充值' }}</td><td class="p-3">{{ order.payment_type }}</td><td class="p-3 text-right font-medium tabular-nums">{{ money(order.amount) }}</td><td class="p-3"><span class="rounded-full bg-emerald-50 px-2 py-1 text-xs text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">{{ statuses[order.status] || order.status }}</span></td><td class="p-3 text-xs">{{ date(order.created_at) }}</td><td class="p-3 text-xs">{{ date(order.paid_at) }}</td></tr></tbody>
        </table>
...
const section = ref<HTMLElement>(), selection = ref<Selection | null>(null)
...
async function open(value: Omit<Selection, 'start_date' | 'end_date'> & Partial<Pick<Selection, 'start_date' | 'end_date'>>) {
  ...
  void load(); await nextTick(); section.value?.scrollIntoView({ behavior: 'smooth', block: 'start' }); section.value?.focus({ preventScroll: true })
}
function close() { controller?.abort(); selection.value = null }
function changePage(delta: number) { page.value += delta; void load() }
watch(() => [props.startDate, props.endDate, props.timezone], close)
