...
describe('user KeysView column settings', () => {
  ...
  describe('create provider selection', () => {
    ...
    it('clears the previous group on provider change and submits only the newly selected group', async () => {
      ...
      expect(vi.mocked(keysAPI.create).mock.calls[0].slice(0, 3)).toEqual(['My key', 'deepseek', 5])
    })
    ...
    it('preserves same-platform multi-group bindings and clears them when changing provider', async () => {
      getAvailableGroups.mockResolvedValue([
        ...availableGroups,
        { ...availableGroups[0], id: 20, name: 'Second Anthropic group' },
      ])
      const wrapper = await openCreate()
      await wrapper.get('[data-tour="key-form-name"]').setValue('Multi-group key')
      await groupSelect(wrapper).vm.$emit('update:modelValue', 1)
      await nextTick()
      const extraGroups = wrapper.get('[data-test="key-additional-groups"]')
      await extraGroups.findAll('input[type="checkbox"]')[1].setValue(true)
      await wrapper.get('#key-form').trigger('submit')
      await flushPromises()
      const args = vi.mocked(keysAPI.create).mock.calls[0]
      expect(args.slice(0, 3)).toEqual(['Multi-group key', 'anthropic', 1])
      expect(args[9]).toEqual([1, 20])
      ...
      await groupSelect(wrapper).vm.$emit('update:modelValue', 11)
      expect(groupSelect(wrapper).props('modelValue')).toBe(11)
    })
