    ...
    <template v-else>
      ...
      <div
        ...
        :class="[
          ...
          selectable && isRowSelected(row, index) ? 'border-primary-300 bg-primary-50/40 dark:border-primary-700 dark:bg-primary-900/10' : '',
        ...
        <template v-else>
          ...
          <tr
            ...
            :class="[
              ...
              selectable && isRowSelected(sortedData[virtualRow.index], virtualRow.index) ? 'bg-primary-50/40 dark:bg-primary-900/10' : '',
          ...
          >
            <td v-if="selectable" class="w-11 min-w-11 px-3 py-4 text-center">
              <input
                type="checkbox"
                class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-800"
                :checked="isRowSelected(sortedData[virtualRow.index], virtualRow.index)"
                :aria-label="getRowSelectionLabel(sortedData[virtualRow.index], virtualRow.index)"
                data-test="select-row"
                @click.stop
                @change="toggleRowSelection(sortedData[virtualRow.index], virtualRow.index, ($event.target as HTMLInputElement).checked)"
              />
            </td>
            <td
