      ...
      <template v-if="isAdmin">
        ...
        <div class="sidebar-section">
          <template v-for="item in adminNavItems" :key="item.path">
            ...
            >
              <Icon v-if="item.iconName" :name="item.iconName" size="md" class="flex-shrink-0" />
              ...
              <Icon v-else name="link" size="md" class="flex-shrink-0" />
...
import { normalizeCustomMenuIconName, type CustomMenuIconName } from '@/components/icons/registry'
    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5m4.75-11.396a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M5 14.5l-2.69 2.689c-1.232 1.232-.65 3.318 1.067 3.611A48.309 48.309 0 0012 21c2.773 0 5.491-.235 8.135-.687 1.718-.293 2.3-2.379 1.067-3.61L19.8 15.3M7.5 15h9'
        })
      ]
    )
}
...
function customMenuNavItem(item: CustomMenuItem): NavItem {
  const iconName = normalizeCustomMenuIconName(item.icon)
  const openInNewTab = item.open_mode === 'new_tab' && isHttpUrl(item.url)
  return {
    path: `/custom/${item.id}`,
    label: item.label,
    iconName: iconName || undefined,
    iconSvg: item.icon_svg,
    externalUrl: openInNewTab ? item.url.trim() : undefined,
  }
}
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/canvas/', label: t('nav.infiniteCanvas'), icon: PlaygroundIcon, externalUrl: '/canvas/', featureFlag: flagInfiniteCanvas },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/operations', label: t('nav.operationsAnalysis'), icon: ChartIcon, hideInSimpleMode: true },
    ...
    },
    {
      ...
      children: [
        ...
        { path: '/admin/orders/campaigns', label: t('nav.rechargeCampaigns'), icon: GiftIcon },
