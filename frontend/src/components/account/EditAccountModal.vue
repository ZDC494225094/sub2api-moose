  ...
  >
    ...
    >
      ...
      >
        ...
        <div
          v-if="upstreamBillingAutoProbeEnabled && account?.platform === 'openai' && !isGiteeAI"
          class="mt-4 max-w-xs"
        >
          <label class="block">
      ...
      <!-- OpenAI API 长上下文计费开关 -->
      <div
        v-if="account?.platform === 'openai' && !isGiteeAI && !isSparkShadow && !hideAccountLongContextBilling && (account?.type === 'oauth' || account?.type === 'setup-token' || account?.type === 'apikey')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between gap-4">
          <div>
        ...
        if (!openAITextGenerationCapabilityEnabled.value || openAIResponsesMode.value === 'auto') {
          ...
          newExtra.openai_responses_mode = openAIResponsesMode.value
			...
			newExtra.upstream_billing_probe_auto_sync_rate_multiplier =
				upstreamBillingAutoProbeEnabled.value && upstreamBillingRateSyncEnabled.value
		}
