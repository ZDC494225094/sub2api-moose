...
describe('validateIntervals', () => {
  ...
  describe('image / per_request / video mode', () => {
    it('allows multiple unbounded tiers identified by label', () => {
      ...
      expect(validateIntervals([
        makeInterval({ tier_label: '480p', per_request_price: 0.04 }),
        makeInterval({ tier_label: '720p', per_request_price: 0.06 }),
        makeInterval({ tier_label: '1080p', per_request_price: 0.08 }),
      ], 'video', t)).toBeNull()
    })
